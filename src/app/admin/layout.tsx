...
const roleBadgeStyles: Record<string, { label: string; className: string }> = {
  super_admin: { label: "Super Admin", className: "bg-gradient-to-r from-amber-100 to-orange-100 text-amber-900 border border-amber-300" },
  admin: { label: "Admin", className: "bg-gradient-to-r from-violet-100 to-fuchsia-100 text-violet-900 border border-violet-300" },
  manager: { label: "Manager", className: "bg-gradient-to-r from-sky-100 to-cyan-100 text-sky-900 border border-sky-300" },
  rep: { label: "Representative", className: "bg-gradient-to-r from-emerald-100 to-teal-100 text-emerald-900 border border-emerald-300" },
};
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      try {
        ...
        // 🔥 Agar cookie nahi hai (401) toh login page par bhejo
        if (!response.ok) {
  ...
  // 🔥 Login page ke liye sirf children (sidebar ke bina)
  if (pathname === "/admin/login") {
    return <>{children}</>;
  }
      ...
      <aside className="w-64 bg-gradient-to-b from-sky-950 via-sky-900 to-sky-950 text-white flex flex-col fixed h-full justify-between z-10 border-r border-sky-900/50">
        ...
        <div className="p-4 border-t border-gray-800">
          <button
            ...
            className="flex items-center gap-3 px-4 py-3 w-full rounded-lg text-sm font-medium text-red-400 hover:bg-red-500/10 hover:text-red-300"
          >
      ...
      <main className="flex-1 ml-64 p-6 md:p-8">
        <header className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            <span className={`px-4 py-1.5 text-xs font-bold uppercase rounded-full ${roleBadge.className}`}>
              {roleBadge.label}
            </span>
...
//   useEffect(() => {
//     // Login page par ho toh session check mat karo
//     if (pathname === '/admin/login') {
//       setLoading(false);
//       return;
//     }
...
//         // 🔥 Agar 401 (unauthorized) hai, toh seedha login page par bhejo
//         if (!response.ok) {
//           router.push('/admin/login');
//           return;
//         }
...
//   // 🔥 Login page ke liye sirf children render karo (sidebar ke bina)
//   if (pathname === '/admin/login') {
//     return <>{children}</>;
//   }
