...
export default function ChatWidget() {
  ...
  // 🆕 Listen for backend proactive trigger (from useVisitorTracking)
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      // Open widget with backend message
      setIsOpen(true);
  ...
  // 🆕 Reset backend trigger flag when user closes widget
  useEffect(() => {
    if (!isOpen) {
      // Don't reset trigger flag — user ko dobara popup nahi chahiye
      // (backend already sets TRIGGERED[session_id]=True)
    }
  }, [isOpen]);
  ...
  // Seed contextual opener on open — SKIP if messages already set (from trigger)
  useEffect(() => {
...
// const AUTO_OPEN_DELAY_MS = 1500;
// const CARD_SHOW_DELAY_MS = 4000; // 4 sec baad card dikhega
...
//   useEffect(() => {
//     if (cardDismissed || isOpen || cardShownRef.current) return;
//     if (isMobile) return; // Skip on mobile (too intrusive)
...
//     if (intent.score >= INTENT_THRESHOLD_FIRE) {
//       autoOpenFiredRef.current = true;
//       // Card hide karo agar khula hua hai
//       setShowCard(false);
//       setTimeout(() => {
//         setIsOpen(true);
//         setHasAutoOpened(true);
